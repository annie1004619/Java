...
public class MonsterGameFrame extends JFrame {	
	...
	public MonsterGameFrame() {
		//학번, 이름 -MonsterGameFrame으로 title
		super("1711412 김지원 -MonsterGameFrame으로 title");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		add(gamePanel); // GamePanel을 컨텐트팬으로 사용한다.
	...
	class GamePanel extends JPanel {
		...
		public GamePanel(String avatarChar, String monsterChar, char quitChar, long monsterDelay) {
			...
			this.monsterChar = monsterChar; //M
			this.quitChar = quitChar; //q
			this.monsterDelay = monsterDelay; //200
		...
		class MyKeyListener extends KeyAdapter {
			...
			public void keyPressed(KeyEvent e) {
				...
				avatar.getParent().repaint(); // 아바타의 위치가 변경되었으므로 다시 그리기  //아바타의 부모는 판넬
	...
	class MonsterThread extends Thread {
		private JLabel from; // 좇아가는 레이블. monster label
		private JLabel to; // 도망가는 레이블. avatar label
		private long monsterDelay;
		...
		@Override
		public void run() {
			int x =from.getX(); // 현재 괴물의 x 좌표
			int y =from.getY(); // 현재 괴물의 y 좌표
			
			while(true) {
				// 아바타가 괴물의 왼쪽이 있는 경우 
					// 괴물의 위치를 MONSTER_MOVE만큼 옮기기 -> 아바타에 가까워지도록 (왼쪽으로)
				if(to.getX()<x)
					x-=MONSTER_MOVE;
				// 아바타가 괴물의 오른쪽이 있는 경우
					// 괴물의 위치를 MONSTER_MOVE만큼 옮기기 -> 아바타에 가까워지도록 (오른쪽으로)
				else
					x+=MONSTER_MOVE;
				// 아바타가 괴물의 위쪽이 있는 경우
					// 괴물의 위치를 MONSTER_MOVE만큼 옮기기 -> 아바타에 가까워지도록 (위쪽으로)
					
				if(to.getY()>y)
					y+=MONSTER_MOVE;
				// 아바타가 괴물의 아래쪽이 있는 경우
					// 괴물의 위치를 MONSTER_MOVE만큼 옮기기 -> 아바타에 가까워지도록 (아래쪽으로)
